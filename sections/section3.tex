...
Section \ref{sec:model}, we will now concretize those ideas and speak about
TensorFlow programming interface. We begin with a brief review of the available
language interfaces in subsection \ref{sec:code-interfaces}. Then, we provide a
more hands-on look at TensorFlow's Python API by walking through a simple
practical example. Lastly, we give insight into what higher-level abstractions
exist for TensorFlow's API, which are especially suitable for rapid prototyping
